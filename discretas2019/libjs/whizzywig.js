...
//Copyright й 2005, John Goodman - john.goodman(at)unverse.net  *date 060201
...
function cleanUp(){  //clean up crud inserted by Micro$oft Orifice
 ...
 h = h.replace(/<\/?o:[^>]*>/gi, "");
 h = h.replace(/<\/?st1:[^>]*>/gi, "");
 h = h.replace(/Ц/g,'-'); //long -
 h = h.replace(/[СТ]/g, "'"); //single smartquotes СТ 
 h = h.replace(/[УФ]/g, '"'); //double smartquotes УФ
